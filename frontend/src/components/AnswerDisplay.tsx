...
export const AnswerDisplay: React.FC<AnswerDisplayProps> = ({ answer, confidence }) => {
  ...
  const getConfidenceColor = () => {
    if (!available) return 'text-gray-500';
  ...
  const getConfidenceIcon = () => {
    if (available && confidence > 0.7) return <CheckCircle2 className="w-5 h-5" />;
    ...
    <div className="bg-white rounded-2xl shadow-xl p-6">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={`flex items-center gap-2 ${getConfidenceColor()} font-semibold`}>
          {getConfidenceIcon()}
          <span>{available ? `${(confidence * 100).toFixed(0)}% Confident` : 'Confidence pending'}</span>
        </div>
      </div>
